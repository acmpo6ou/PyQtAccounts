...
class DbSaveTest(AccsTest):
    ...
    def test_save_after_edit(self):
        ...
        self.assertEqual(
            "Ім'я: Ross Geller",
            accs.forms["show"].name.text(),
            "Name of account is not changed after database was" "saved!",
        )
    ...
    def test_save_message_Yes(self):
        ...
        self.assertNotIn(
            self.win,
            self.window.windows,
            "Database window is closed but it is still in windows" "list!",
        )
        self.window.close()
        del self.window

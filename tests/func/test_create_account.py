...
class CreateAccountTest(AccsTest):
    ...
    def checkNameErrors(self):
        ...
        """
        self.assertFalse(
            self.nameError.visibility, "Name error is visible when it shouldn't be!"
        )
        self.assertFalse(
    ...
    def test_create_button_enabled(self):
        ...
        self.assertFalse(
            ...
            "Create button is not disabled when create form is just" "opened!",
        )
        ...
        self.assertFalse(
            self.createButton.isEnabled(),
            "Create button is not disabled when password fields" "aren't filled!",
        )
        ...
        self.assertFalse(
            self.createButton.isEnabled(),
            "Create button is not enabled when name field is" "cleared!",
        )
        ...
        self.assertFalse(
            self.createButton.isEnabled(),
            "Create button is not disabled when password fields " "are both erased!",
        )
    ...
    def test_create_button(self):
        ...
        self.assertEqual(
            self.email_radio.text(),
            "E-mail",
            "Text of e-mail radio button is incorrect!",
        )
        self.assertEqual(
            self.username_radio.text(),
            "Username",
        ...
        self.assertEqual(
            "someaccount", acc.account, "account field of created account is incorrect!"
        )
        self.assertEqual(
            "somename", acc.name, "name field of created account is incorrect!"
        )
        self.assertEqual(
            b"some_password",
            acc.password,
        ...
        self.assertEqual(
            False, acc.copy_email, "copy_email field of created account is incorrect!"
        )
        self.assertEqual(
            "01.01.2000", acc.date, "date field of created account is incorrect!"
        )
        self.assertEqual(
            "Comment of account.",
            acc.comment,
    ...
    def test_attach_files(self):
        ...
        self.monkeypatch.setattr(QFileDialog, "getOpenFileName", self.mock_browse(""))
        self.attach_file_button.click()
        ...
        # And it is in the database and has all keys and values
        self.assertIn(
            "tf2", self.win.db, "Account has not appeared in database after creation!"
        )
        acc = self.win.db["tf2"]
        self.assertEqual(
            "tf2", acc.account, "account field of created account is incorrect!"
        )
        self.assertEqual(
            b"something",
            acc.password,
        ...
        expected_attached_files = {"somefile.txt": b"Some another file.\n<h1></h1>\n"}
        self.assertEqual(
